...
Treap 是一种比较基础的平衡树，它的名字是树（Tree）和堆（Heap）的合成词，就是说它即满足 BST 的性质也满足二叉堆的性质。

具体来说，我们在每插入一个结点时，除了它作为 BST 的权值，再给它随机一个作为二叉堆的权值，因为在随机数据下堆的期望深度是 $O(\log n)$ 的。然后我们考虑通过某种方法使它既满足 BST 的性质也满足二叉堆的性质。
...
FHQ Treap 码量比旋转 Treap 小，写起来更直观，更好理解，但缺点是常熟略大。鉴于现在 CCF 在尽力减小常数对考生成绩的影响，我更推荐学习 FHQ Treap。
...
分裂操作有两种，按值分裂和按大小分裂，这里因为我的 Treap 不需要按大小分裂就不提了。
...
![图示 5](../../pictures/Treap_5.png)

考虑分裂的过程是什么样的。

首先从根节点进入，发现根节点 $8>6$，说明根节点及其整个右子树都属于 $T_2$，而左子树里较大的可能也属于 $T_2$，继续往左子树分离。
