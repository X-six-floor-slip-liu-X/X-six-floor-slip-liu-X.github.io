...
这里就不能每次试跳 $fail$ 了，因为 $L$ 和 $N$ 同阶。考虑优化。发现假如连结点数 $n$ 条 $(fail_i,i)$ 的无向边，会构成一棵以 Trie 树的根为根的 $fail$ 树的形式，发现每个状态匹配数就是 $fail$ 树上它的子树内所有点被经过次数总和，这样复杂度就是 $O(\sum|s_i|+n|\Sigma|+S+n)$ 的。
