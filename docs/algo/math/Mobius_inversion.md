...
数论函数指定义域是正整数域 $\mathbb{Z}^+$，陪域（大概能理解为值域的超集）是复数域 $\mathbb{C}$ 的函数（但常见的的数论函数值域一般都是整数集合的子集）。

### 狄利克雷卷积

狄利克雷卷积是一种定义在数论函数上的运算，$f$ 和 $g$ 的狄利克雷卷积通常用 $f*g$ 来表示（就是星号 `*`，不是点 `\cdot`）。有 $(f*g)(n)=\sum_{xy=n}f(x)g(y)$，或者写作 $(f*g)(n)=\sum_{d\mid n}f(d)g(\frac{n}{d})$。

它有几乎和乘法一样的性质：

- **交换律**：$f*g=g*f$，显然。
- **结合律**：$(f*g)*h=f*(g*h)$，证明非常简单，因为 $\left((f*g)*h\right)(n)=\sum_{xyz=n}f(x)g(y)h(z)$，与运算顺序无关。
- **分配率**：$(f+g)*h=f*h+g*h$，其中加法表示单点函数值相加。证明也考虑套定义，略过。

以及一个神秘性质（虽然乘法也有这个性质，但证明方法不太一样）：

- $f=g\Leftrightarrow f*h=g*h$，其中数论函数 $h(x)$ 要满足 $h(1)\ne 0$。
...
设 $r=f*h-g*h=(f-g)*h$，那么 $r(x)=\sum_{d\mid x}(f(d)-g(d))h(\frac{x}{d})$。
...
- **元函数**：通常用 $\epsilon$（`\epsilon`）表示，$\epsilon(n)=[n=1]$，对于任意数论函数 $f$ 均有 $\epsilon*f=f$，原因带入定义证即可。
...
另外，由于单位元的存在，我们可以对数论函数定义狄利克雷逆。通常用 $f^{-1}$ 表示 $f$ 的狄利克雷逆，意义是 $f*f^{-1}=f^{-1}*f=\epsilon$，根据 $f=g\Leftrightarrow f*h=g*h$ 的性质，同一个数论函数的狄利克雷逆唯一。
...
首先必要性比较简单，考虑否命题（即逆命题的逆否命题，若它为真那么逆命题为真，必要性成立）是当 $f(1)=0$，不存在狄利克雷逆。因为 $\epsilon(1)=(f*f^{-1})(1)=f(1)f^{-1}(1)=1$，当 $f(1)=0$，这个方程无解。

然后考虑充分性，考虑从小到大构造 $f^{-1}$，注意到 $(f*f^{-1})(x)=\sum_{d\mid x}f(d)f^{-1}(\frac{x}{d})$ 中，除了 $d=1$ 都是确定的，那么构造如下：

$$
f^{-1}(x)=\frac{\epsilon(x)-\sum_{d\mid x}^{d\ne 1}f(d)f^{-1}(\frac{x}{d})}{f(1)}
$$

这样就满足 $(f*f^{-1})(x)=\epsilon(x)$ 了。显然当 $f(1)\ne 1$ 时均能构造出来，并且直接暴力跑复杂度甚至是 $O(n\log n)$（调和级数）的。
...
\begin{aligned}
f^{-1}(nm)
&=\frac{\epsilon(nm)-\sum_{d\mid nm}^{d\ne 1}f(d)f^{-1}(\frac{nm}{d})}{f(1)}\\\\
...
&=f(1)f(1)g(n)g(m)-(f*f^{-1})(n)(f*f^{-1})(m)\\\\
&=f^{-1}(n)f^{-1}(m)-\epsilon(n)\epsilon(m)\\\\
&=f^{-1}(n)f^{-1}(m)
\end{aligned}
$$

其中最后一步能直接去掉 $\epsilon(n)\epsilon(m)$ 是因为 $n,m$ 中至少一个不为 $1$。
...
考虑很多时候可能存在 $F=f*I$ 的关系，并且 $F$ 是好求的而 $f$ 不好求。通过 $f$ 求 $F$ 是简单的，可以 $O(n\log n)$ 暴力，那根据 $F$求 $f$ 呢？根据反演的套路，可以考虑用 $f=F*I^{-1}$ 来求出 $f$。因为 Möbius 把 $I^{-1}$ 命名为 $\mu$（莫比乌斯函数），所以这样的反演叫莫比乌斯反演（虽然有时候用其它函数的狄利克雷逆做的反演也叫莫比乌斯反演）。

首先显然地，$\mu$ 是一个积性函数。对于积性函数通常先考虑它在质数的次幂时的表现，那么我们先考虑 $\mu(p^k)$，其中 $p$ 是质数。

对于 $k=0$ 是平凡的，$\mu(1)=1$。

当 $k=1$ 时，$\epsilon(p)=\sum_{d\mid p}I(d)\mu(\frac{p}{d})=I(1)\mu(p)+I(p)\mu (1)=0$，则 $\mu(p)=-1$。
...
证明非常显然，左边是 $\mu *I=\epsilon$，右边就是 $\epsilon$。
