...
- 有 $q$ 次修改，每次单点修改点权，每次修改后求 $\min_{u}\left\{\max_{v}d(u,v)\right\}$。
...
假设点 $u$ 周围的点 DP 值都确定了，因为是最坏情况，所以需要将所有转移的最小值与 $val_u$ 取 $\max$，即 $dp_u=\max(val_u,\min_{(u,v)\in E}\left\{dp_v+w(u,v)\right\})$，其中 $w(u,v)$ 代表 $u,v$ 的边权。

然后把 $\min,\max$ 交换可得 $dp_u=\min_{(u,v)\in E}\left\{\max(val_u,dp_v+w(u,v))\right\}$，容易发现所有转移都是从较小的转移到较大的，故不会形成环，于是可以类似 dij 地做这个过程，复杂度 $O(n\log n)$。
...
一个想法是先确定每个点的深度，然后从底层到高层一层一层确定。假如在处理第 $i$ 层，考虑钦定一个 $i-1$ 层的点 $u$，对于下一层的点 $v$，可以询问它和 $u$ 的距离找到 $\mathrm{lca}(u,v)$，从 $\mathrm{lca}(u,v)$ 的另一个子树进去就是一个子问题了。

那么如何让这个过程的复杂度正确呢？考虑树剖，让 $u$ 为根节点的末端，这样这个过程就只会进行 $O(\log)$ 次了（就是树剖的复杂度）。
