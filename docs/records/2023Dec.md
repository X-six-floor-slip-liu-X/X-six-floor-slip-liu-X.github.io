...
容易发现，在 $sa$ 上，对于一个固定的 $u$，$\mathrm{lcp}(u,v)$ 是关于 $v$ 单峰的，其中在 $v=u$ 时取到唯一一个最大值（显然吧？区间最小值在固定某端点时肯定关于另一端点显然单调不增）。也就是说，能取到 $\mathrm{lcp}(u,v)\ge x$ 的 $v$ 是一个连续的区间。在确定答案为 $mid$ 后，只要查询区间 $[a,b-mid+1]$ 中有没有 $rk_i$ 在能取到 $\mathrm{lcp}(a,v)\ge mid$ 的 $v$ 的区间里即可。这个区间可以 RMQ+二分得到，然后可以用主席树维护，就做完了，复杂度 $O(n\log n+m\log^2n)$。
...
由于 $s_{i+1}$ 必须是 $s_i$ 的子串，所以必定是 $s_i$ 某个前缀的后缀，即读入 $s_i$ 过程中经过的某个状态的祖先，那么就可以在 parent tree 上从上到下转移。这个可能会想到树上倍增转移，但是一个聪明的方法是除去 $dp_x$ 外再记一个 $g_x$ 表示 $x$ 的祖先中，$dp$ 值与 $x$ 相同的最高点。容易发现 $dp_v$ 要么等于 $dp_x$，要么等于 $dp_x+1$，即从某个等于 $dp_x$ 的位置转移过来。而如果 $x$ 到 $g_x$ 的链上有某个状态在 $v$ 中匹配了至少两次，那么 $g_x$ 必定也在 $v$ 中出现了至少两次。所以直接从 $dp_x$ 转移时找 $\mathrm{endpos}_{\mathit{g_x}}$ 在 $[pos_v-\mathrm{len_{\mathit{v}}}+\mathrm{len_{\mathit{g_x}}},pos_v]$ 中有没有出现至少两次即可（其中 $pos_v$ 是在 $v$ 的 $\mathrm{endpos}$ 中随便找的一个位置），这个可以用线段树的可持久化合并维护。

然后转移边的转移就直接取 $\max$ 即可。注意更新 $g$。

然后考虑转移顺序。其实很简单，容易发现把 parent tree 和转移边放一起仍然是个 DAG，因为每条边都是从更短的 $\mathrm{len_{\mathit{p}}}$ 指向更长的 $\mathrm{len_{\mathit{q}}}$ 的。那么按拓扑序即可。
...
容易发现本质不同子串个数就是每个等价类的 $\mathrm{len}-\mathrm{minlen}+1=\mathrm{len}-\mathrm{len_{link}}$。
