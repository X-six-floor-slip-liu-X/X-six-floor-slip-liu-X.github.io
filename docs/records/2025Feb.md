...
[传送门](https://www.luogu.com.cn/problem/P3287)

> 题意

- 给定一个长度为 $n$ 的整数序列 $a$。
- 你可以进行至多 $k$ 次区间 $+1$ 操作，最大化 LIS 长度。
- $1\le n\le 1\times 10^4,1\le k\le 500,1\le a_i\le 5000$

> 题解

简单题。

首先不难发现每次操作一个后缀是不劣的（因为这个后缀的任意前缀都不如这个后缀）。

于是修改的影响容易维护，就不难 DP 了。设 $f_{i,j}$ 表示考虑 $[1,i]$，操作 $j$ 次后以 $i$ 结尾的最大 LIS 是多少。

$f_{i',j'}\to f_{i,j}$ 转移有两个限制：$j'\le j,a_{i'}+j'\le a_i+j$，可以用二维树状数组维护。

复杂度 $O(nk\log k\log V)$。
...
#undef gc
const int N=1e4+5,M=505,V=5005,inf=0x3f3f3f3f;
int n,m,a[N],mx;
struct BIT{
	int c[V+M];
	void upd(int x,int k){for(;x<=mx;x+=x&-x)c[x]=max(c[x],k);}
	int sum(int x){int res=0;for(;x>0;x-=x&-x)res=max(res,c[x]);return res;}
};
struct BIT2{
	BIT t[M];
	void upd(int x,int p,int k){++x;for(;x<=m+1;x+=x&-x)t[x].upd(p,k);}
	int sum(int x,int p){int res=0;++x;for(;x>0;x-=x&-x)res=max(res,t[x].sum(p));return res;}
}mt;
int f[N][M];
signed main(){
	n=read();m=read();
	...
	forup(i,1,n){
		fordown(j,m,0){
			f[i][j]=mt.sum(j,a[i]+j)+1;
			msg("%d ",f[i][j]);
			mt.upd(j,a[i]+j,f[i][j]);
		}
		msg("|\n");
		ans=max(ans,f[i][m]);
	}
...
[传送门](https://www.luogu.com.cn/problem/CF1098D)

> 题意

- 有一个鱼缸，初始是空的。
- 有 $q$ 次放入或捞出一条大小为 $x$ 的鱼的操作。
- 鱼缸里的鱼会互相吃，具体来说，若两条鱼的大小 $a\le b$，那么 $b$ 就能吃掉 $a$，并且大小变成 $a+b$。
- 我们定义一次吃鱼是危险的，当且仅当 $a\le b$ 且 $2a\ge b$。在每次操作后，求如果让这些鱼互相吃直到只剩一只，最多会有多少次危险的吃鱼。注意鱼并不会真的互相吃，这只是一个假设情况。
...
首先不难观察出答案的上界：将所有鱼从小到大排序得到序列 $p$ 以及前缀和 $s$，那么 $2s_{i-1} < p_i$ 的点绝对不可能产生贡献，其余点或许都有可能产生贡献。

那么这个上界能否达到呢？有非常简单的构造：每次把最小的两条鱼拿出来合并即可。考虑证明。

我们只需要证明对于一次不危险的合并 $a,b(2a < b)$，$b$ 必定从未被合并过。考虑反证若 $b$ 被合并过，不妨设是由 $c+d$ 得到的，那么 $\max(c,d)\ge \frac{b}{2} > a$，与从小到大合并的条件矛盾。那么就真的只有上述绝对不可能产生贡献的点是不危险的。

怎么维护呢？考虑将值域倍增分块，即分为 $[2^0,2^1),[2^1,2^2),\dots$，那么每一块中最小的不小于最大的的一半，这样每一块就只有第一个点有可能不危险了，用 `std::set` 维护每一块的最小值，再维护每一块的和即可。复杂度 $O(n(\log n+\log V))$。
...
#undef gc
const i64 N=5e5+5,inf=0x3f3f3f3f;
i64 n,cnt;
i64 psum[35];
multiset<i64> mn[35];
i64 calc(){
...
[传送门](https://www.luogu.com.cn/problem/CF1408H)

> 题意

- 给定一个长度为 $n$ 的整数序列 $p_i$，值域为 $[0,n]$。
- 求至多能划分为多少个三元组 $(a_i,b_i,c_i)$，满足以下要求：
    - $a_i < b_i < c_i$
    - $p_{a_i}=p_{c_i}=0,p_{b_i}\ne 0$
    - 对于 $i\ne j$，$p_{b_i}\ne p_{b_j}$。
    - 任意一个下标至多使用一次。
- $1\le n\le 5\times 10^5$，带多测。

> 题解

妙妙题。

设 $p$ 中 $0$ 的个数为 $c$，显然答案不会超过 $\frac{c}{2}$。那么若一个非 $0$ 点左侧有不少于 $\frac{c}{2}$ 个 $0$，那么我们其实就只需要关注其右侧的限制了，右侧也是同理。最终我们将这个更弱的问题的答案和 $\frac{c}{2}$ 取 $\min$ 即可。

假如每个颜色只出现了至多一次，那么是好做的，所有左限制点互相竞争左限制，右限制点互相竞争右限制，排序后贪心匹配即可。

出现多次怎么办呢？首先显然我们只需要分别保留每个颜色在左右限制中限制最松的。不妨假设最初所有有左限制的颜色全都去竞争左限制，仿照上面的贪心即可，显然会有 $0$ 的个数不足的情况，此时我们就只能委屈当前用过的颜色中**有右限制，且右限制最松的**去竞争右限制了，再对右限制贪心。这显然是能算到最大值的。

因为要排序，还要 `std::set` 维护算左限制时的最松右限制，复杂度 $O(n\log n)$。
...
void solve(){
    ...
    for(auto i:vl){
        if(pre[i]>cnt1){
            if((cnt1+1)*2>pre[n]) break;
            ++cnt1;
            if(pr[a[i]]!=n+1) ss.insert(pr[a[i]]);
        }else{
            if(pr[a[i]]!=n+1) ss.insert(pr[a[i]]);
            if(ss.empty()) continue;
            int u=*ss.begin();ss.erase(ss.begin());
            vr.push_back(u);
        }
    }
...
[传送门](https://www.luogu.com.cn/problem/P10218)

> 题意

- 给定两个长度为 $n$ 的整数序列 $a,b$，满足 $a_i\in[0,2^{k}-1],b_i\ge 0$，你需要出 $S\subseteq\begin{Bmatrix}1,2,3,\dots n\end{Bmatrix}$ 以及 $x\in[0,2^{k}-1]$ 满足 $\sum_{i\in S}b_i\le m$。
- 在此基础上，最大化 $\min(\min_{i\in S}(a_i+x),\min_{i\notin S}(a_i\oplus x))$，此处 $\oplus$ 指按位异或。
- $1\le n\le 10^5,0\le m\le 10^9,0\le k\le 120,0\le b_i\le 10^9$

> 题解

贪心。

首先能明确的是 $a+b\ge a\oplus b$，那么若 $\sum b_i\le m$，那么全部加入 $S$ 即可，答案就是 $\min(a_i+2^{k+1}-1)$。其余情况必定有需要异或的部分。

看到异或就该想到 Trie 吧。考虑从高到低贪心确定答案的每一位能否为 $1$，假如某个点只有一个儿子显然答案这一位是可以为 $1$ 的，否则必定其中只有一个儿子的这一位能异或得到 $1$，另一个儿子需要通过加法得到。

于是此时我们可以刻画出若 $x$ 这一位取 $0$，答案这一位能为 $1$ 的条件：

1. 左子树（$0$ 子树）内 $b_i$ 之和小于等于剩下的 $m$。
1. 当前已经加入 $S$ 的数最小值加上 $x$（更低位取 $1$，即此后能取到的最大值）能大于等于现在的答案（更低位取 $0$，即此后的最小值）。

满足这个条件后，左子树的贡献就是确定的，直接统计上左子树的影响，向右子树递归。$x$ 这一位取 $0$ 是对称的。

若不满足，那么这一位异或后取到 $1$ 的部分就不会贡献了，只需要往其中有一边递归。需要明确的是，若答案的这一位能取 $1$ 那么取 $0$ 就必定更劣了，无需往下递归。

这样每次最多只会向两个儿子各递归一次，那么每个点至多被访问一次。复杂度 $O(n\log V)$。

注意各种 `__int128` 以及各种空间问题。
...
#undef gc
void print128(i128 x,bool flag=true){
    if(x==0){
        if(flag){
            putchar('0');
        }
...
struct Trie{
    ...
    void Insert(i128 a,int b){
        ...
        fordown(i,k-1,0){
            ...
            p=tr[p][c];
            mn[p]=min(mn[p],a);
            sum[p]=min(sum[p]+b,m+1);
        }
    }
    void dfs(int d,int u,i128 mnS,i128 x,i64 m,i128 ans){
        if(d==-1){
            res=max(res,ans);
            return;
        }
        if(!u){
            res=max(res,mnS+(x|(((i128)1<<(d+1))-1)));
            return;
        }
        bool flag=false;
        if(m>=sum[tr[u][1]]&&min(mnS,mn[tr[u][1]])+(x|(((i128)1<<(d+1))-1))>=(ans|((i128)1<<d))){
            dfs(d-1,tr[u][0],min(mnS,mn[tr[u][1]]),x|((i128)1<<d),m-sum[tr[u][1]],ans|((i128)1<<d));
            flag=true;
        }
        if(m>=sum[tr[u][0]]&&min(mnS,mn[tr[u][0]])+(x|(((i128)1<<d)-1))>=(ans|((i128)1<<d))){
            dfs(d-1,tr[u][1],min(mnS,mn[tr[u][0]]),x,m-sum[tr[u][0]],ans|((i128)1<<d));
            flag=true;
        }
        if(flag) return;
        dfs(d-1,tr[u][1],mnS,x|((i128)1<<d),m,ans);
        dfs(d-1,tr[u][0],mnS,x,m,ans);
        return;
    }
}mt;
void solve(){
...
[传送门](https://www.becoder.com.cn/contest/6058/problem/3)

> 题意

- 有一个 $1\times n$ 的棋盘，我们用一个整数 $i$ 表示从左往右第 $i$ 个格子，Alice 和 Bob 正在这个棋盘上放棋子，Alice 执黑（$0$），Bob 执白（$1$）。
- 两个人不一定轮流行动，两个人认为相邻的棋子颜色相同是不好看的，所以在放置时会优先放在相邻格没有自己颜色棋子的位置，当不存在这样的位置时， ~~当前操作的人会举手告诉裁判~~ ，然后此后两人的操作就全都没有这样的限制了。
- 现在给定最终每个格子上棋子的颜色，问有多少种操作序列能到达这个颜色。
- 两个操作序列不同当且仅当某个时刻行动的人不同或者落子位置不同。
- $1\le n\le 10^5$，带多测，$\sum n\le 2\times 10^5$

> 题解

妙妙题。

首先不难想到，整个过程大致是两个人先按规则行动， ~~然后其中一个人突然举手~~ ，然后两人随意乱走。称 ~~举手~~ 前为一阶段，随意随意乱走的部分是二阶段。显然二阶段的方案数是一个阶乘的形式（除去二阶段的第一步，这一步必须由 ~~举手~~ 的那个人走），因为在确定最终状态和每一步放的位置后，这一步是哪个人操作也是确定的。并且假如我们确定了一阶段落子位置的集合，那么一阶段的方案数也是一个阶乘的形式

那么不难得出一个 $O(n^3)$ 的 DP：先假设 ~~举手~~ 的人是 Alice（后面再按 Bob 做一遍即可），那么设 $f_{i,j,k}$ 表示考虑前 $i$ 个格子，选择了 $j$ 个黑子和 $k$ 个白子加入一阶段，并且任意一个空格周围都有至少一个黑子的方案数（只算选择的方案数，不考虑阶乘）。转移只需要枚举 $O(1)$ 的前驱状态，最终统计答案就是先乘上选一个没选过的黑子做转折的方案数，再乘两个阶乘。

考虑我们为什么要分别记录已选白字和黑子的个数，就是因为统计转折操作的位置时需要区分两种棋子。那么能否省去这一步呢？我们可以把转折操作也计入状态。具体来说，设 $f_{i,j,0/1}$ 表示考虑前 $i$ 个格子，选了 $j$ 个棋子，并且是/否在空的黑色格子里选了恰好一个作为转折点的方案数。转移仍然只需要 $O(1)$ 的枚举，就能 $O(n^2)$ 做了。

但是这玩意看起来没啥优化空间，因为转移几乎就是一些点对点的转移了。考虑设一些新的 DP 状态，我们可以将整个序列看作一段一段交替的 $0$ 和 $1$，发现每一段的合法状态数是好算的，（在钦定转折点是黑子的情况下），对于黑色连续段，每个状态形如若干个间隔是 $1$ 或者 $2$ 的黑子，可以直接枚举有多少个长度为 $2$ 的连续段然后组合数求出，总复杂度是黑色段的总长 $O(n)$。对于白色连续段，由于有每个空格相邻格子中至少有一个黑子的限制，那么状态会和相邻的黑子有关，具体如何我们后文再说。总之，根据上面的分析，不难想到一个基于连续段的状态设计：$f_{l,r,0/1,0/1,0/1,j}$ 考虑第 $l$ 个到第 $r$ 个黑色连续段（及其之间的白色连续段），最左侧是/否有棋子，最右侧是/否有棋子，是/否已经选择了转折点，总共已经选了 $j$ 个棋子的方案数。

状态的合并就是通过一个白色连续段将两个黑色连续段连在一起，称左侧段的最右侧为 $X$ 位置，右侧段的最左侧为 $Y$ 位置。于是我们考虑这个白色段的贡献：

1. 白色段长度为 $1$：

    若 $X,Y$ 位置均无黑子，那么这个点必须放一个白子（否则不满足每个空格周围都有黑子的条件）。

    否则（$X,Y$ 中至少有一个黑子时），这个点可放可不放。

2. 白色段长度为 $2$：

    若 $X,Y$ 均无黑子，那么无解。

    若两边都有黑子，那么可以选择在两个位置任意放一个白子或者不妨。
    
    若只有一边有黑子，那么必须在另一边放一颗白子。

3. 白色段长度为 $3$：

    若 $X,Y$ 均有黑子，则在中间放一个白子。

    否则无解。

4. 白色段长度大于 $3$：

    显然无解，无论怎么放中间必定会留左右都无黑子的空格。

不难发现每种转移都相当于两个数列进行卷积，可以用 NTT 进行优化。因为是将若干个相对短的数列卷积起来，考虑分治 NTT，这里不指半在线卷积，而指每次分成两半作为子问题分别计算，再卷积起来。

复杂度 $O(n\log^2 n)$，常数略大，需要酌情卡常，比如预处理 NTT 中的原根以及反转数。
...
#undef gc
const uint N=1e5+5,mod=998244353,inv3=(mod+1)/3;
uint ksm(uint a,uint b){
    uint c=1;
    while(b){
        if(b&1) c=1ull*a*c%mod;
        a=1ull*a*a%mod;
        b>>=1;
    }
    return c;
}
uint n,m,fact[N],finv[N];
uint binom(uint n,uint m){
    if(n<m) return 0;
    return 1ull*fact[n]*finv[m]%mod*finv[n-m]%mod;
}
char str[N];
uint ans;
pii a[N];
vector<uint> dp[N][2][2][2];
uint add(uint x,uint y){
    x+=y;
    if(x>=mod) x-=mod;
    return x;
}
uint del(uint x,uint y){
    if(x<y) x+=mod;
    x-=y;
    return x;
}
namespace NTT{
    ...
    void init(){
        forup(i,0,20){
            rv[i].resize(1<<i);
            forup(j,0,(1<<i)-1){
                rv[i][j]=(rv[i][j>>1]>>1)|(((j&1)<<(i-1)));
            }
            g[i]=ksm(3,(mod-1)/(1<<i));
            ivg[i]=ksm(inv3,(mod-1)/(1<<i));
        }
    }
    void NTT(vector<uint> &f,uint p,uint type){
        uint w=31^__builtin_clz(p);
        f.resize(p);
        vector<int> &rev=rv[w];
        forup(i,0,p-1){
            if(rev[i]<i) swap(f[i],f[rev[i]]);
        }
        for(uint len=1,w=1;len<p;len<<=1,++w){
            uint wn=(type?g[w]:ivg[w]);
            for(uint i=0;i<p;i+=(len<<1)){
                uint nw=1;
                forup(j,0,len-1){
                    uint x=f[i+j],y=1ull*nw*f[i+len+j]%mod;
                    f[i+j]=add(x,y);
                    f[i+len+j]=del(x,y);
                    nw=1ull*nw*wn%mod;
                }
        ...
        if(type==0){
            uint inv=ksm(p,mod-2);
            forup(i,0,p-1){
                f[i]=1ull*f[i]*inv%mod;
            }
        }
    }
    void convolution(const vector<uint> &a,const vector<uint> &b,vector<uint> &c){
        vector<uint> pa=a,pb=b;
        uint p=1;
        while(p<pa.size()+pb.size()) p<<=1;
        pa.resize(p);pb.resize(p);
        if(c.size()<p) c.resize(p);
        NTT(pa,p,1);NTT(pb,p,1);
        forup(i,0,p-1) pa[i]=1ull*pa[i]*pb[i]%mod;
        NTT(pa,p,0);
        c.resize(p);
        forup(i,0,p-1) (c[i]+=pa[i])%=mod;
        while(c.size()&&c.back()==0) c.pop_back();
    }
}
uint calc(uint l,uint r){
    if(l==r){
        forup(L,0,1){
            forup(R,0,1){
                dp[l][L][R][0].resize(a[l].fi+1);
                dp[l][L][R][1].resize(a[l].fi+1);
                int len=a[l].fi-(L==0)-(R==0)-1;
                if(len<0) continue;
                forup(c,0,len/3){
                    if((len-c)&1) continue;
                    uint num=(len-c)/2;
                    (dp[l][L][R][0][num+1]+=binom(num,c))%=mod;
                    (dp[l][L][R][1][num+1]+=1ull*binom(num,c)*(a[l].fi-num-1)%mod)%=mod;
                }
                while(!dp[l][L][R][0].back()) dp[l][L][R][0].pop_back();
                while(!dp[l][L][R][1].back()) dp[l][L][R][1].pop_back();
            }
        }
        return l;
    }
    uint mid=(l+r)>>1;
    if((r-l+2)%4!=0) ++mid;
    uint pl=calc(l,mid-1),pr=calc(mid+1,r);
    uint len=a[mid].fi;
    if(len==1){
        forup(L,0,1) forup(R,0,1) forup(X,0,1) forup(Y,0,1){
            forup(p,0,1) forup(q,0,1){
                if(p&&q) continue;
                vector<uint> &vec=dp[mid][L][R][p|q];
                const vector<uint> &vl=dp[pl][L][X][p],&vr=dp[pr][Y][R][q];
                if(vl.empty()||vr.empty()) continue;
                vector<uint> vp;
                vp.resize(vr.size()+1);
                forup(i,0,vr.size()-1) vp[i+1]=vr[i];
                if(X||Y) forup(i,0,vr.size()-1) (vp[i]+=vr[i])%=mod;
                NTT::convolution(vl,vp,vec);
            }
        }
    }else if(len==2){
        forup(L,0,1) forup(R,0,1) forup(X,0,1) forup(Y,0,1){
            ...
            forup(p,0,1) forup(q,0,1){
                ...
                forup(i,0,vr.size()-1) vp[i+1]=vr[i];
                if(X&&Y){
                    forup(i,0,vr.size()-1) (vp[i+1]+=vr[i])%=mod;
                    forup(i,0,vr.size()-1) (vp[i]+=vr[i])%=mod;
                }
                NTT::convolution(vl,vp,vec);
            }
        }
    }else{
        forup(L,0,1) forup(R,0,1){
            forup(p,0,1) forup(q,0,1){
                if(p&&q) continue;
                vector<uint> &vec=dp[mid][L][R][p|q];
                const vector<uint> &vl=dp[pl][L][1][p],&vr=dp[pr][1][R][q];
                if(vl.empty()||vr.empty()) continue;
                vector<uint> vp;
                vp.resize(vr.size()+1);
                forup(i,0,vr.size()-1) vp[i+1]=vr[i];
...
void work(uint p){
    ...
    if(a[1].se!=p) ++l;
    if(a[n].se!=p) --r;
    if((l==2&&a[1].fi==3)||(r==n-1&&a[n].fi==3)) return;
    uint rr=calc(l,r);
    forup(L,0,1) forup(R,0,1){
        const vector<uint> &vec=dp[rr][L][R][1];
        forup(j,0,vec.size()-1){
            uint tl=0,tr=0,dl=0,dr=0;
            if(l==2){
                if(a[1].fi==1){
                    if(L) tl=0,tr=1;
                    else tl=1,tr=1;
            ...
            forup(l,tl,tr){
                forup(r,dl,dr){
                    uint cc=j+l+r;
                    if(vec[j]) msg("%d %d %d|%d %d\n",L,R,j,cc,vec[j]);
                    (ans+=1ull*fact[cc]*fact[m-cc-1]%mod*vec[j]%mod)%=mod;
                }
            }
        }
    }
...
signed main(){
    ...
    NTT::init();
    fact[0]=1;
    forup(i,1,n) fact[i]=1ull*fact[i-1]*i%mod;
    finv[n]=ksm(fact[n],mod-2);
    fordown(i,n-1,0) finv[i]=1ull*finv[i+1]*(i+1)%mod;
    uint t=read();
    while(t--){
        solve();
    }
...
[传送门](https://www.luogu.com.cn/problem/P10360)

> 题意

- 有一行 $n$ 个点，每个点可能是黑色或白色。
- 有 $m$ 个操作依次进行，每个操作形如：若点 $a$ 是黑点且点 $b$ 是白点，交换 $a,b$。
- 对于每个整数 $k\in[1,n]$，求有多少种初始有 $k$ 个黑点的排列，使得最终黑点组成一个连续的区间。
- **答案对 $2$ 取模**。
- $1\le n\le 35,1\le m\le 1000$

> 题解

挺妙的暴力枚举，但貌似是套路，波兰人都喜欢这样出题吗。

考虑最暴力的做法显然是 $O(2^nm)$ 暴力，但显然过不了。

考虑我们没用到答案对 $2$ 取模的性质。这种题的套路做法是若同一状态出现了两次就能直接删掉，考虑在这道题上怎么做。

首先显然是不能直接枚举初状态了。不妨设每个点初始都是一个未知数 $?$，然后考虑一次操作 $(a,b)$ 会发生什么。

1. 若 $a,b$ 均为 $?$

    现在需要确定 $a,b$ 的值了。容易发现若 $a,b$ 一黑一白那么这个操作之后都会变成 $a$ 白 $b$ 黑。同一个状态出现了两次，不需要考虑。于是只需要考虑两个都是白色或都是黑色的情况了。
...
3. 若 $a,b$ 恰有一个为 $?$
    ...
    分情况讨论一下，不难发现每种情况（$a$ 或者 $b$ 是 $0$ 或者 $1$）无论 $?$ 变成 $1$ 还是 $0$ 都要么不产生影响要么 $?$ 的位置和另一个交换，只有一个后继。

不难发现只有 $1$ 情况会使状态数增加，每次增加状态会消耗两个 $?$，那么状态数不超过 $2^{\frac{n}{2}}$ ，随便怎么写都能过了，我为了方便用了 `std::bitset`，并且最终 $O(2^{\frac{n}{2}}n^2)$ 统计答案，但还是过了。
...
signed main(){
    ...
    for(auto s:f){
        ...
        forup(l,0,n-1){
            if(l&&s1[l-1]) break;
            forup(r,l,n-1){
                if(s1[r]-(l?s1[l-1]:0)==s1[n-1]&&s0[r]-(l?s0[l-1]:0)==0){
                    cnt[r-l+1]^=1;
                }
            }
        }
