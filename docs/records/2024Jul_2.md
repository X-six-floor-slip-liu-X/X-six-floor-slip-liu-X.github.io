...
$$
f_i=\max_{j=i-k}^{k\times(\left\lceil\frac{i}{k}\right\rceil-1)}\left\{f_j+\max_{l\in [j+1,i]}a_l\right\}
$$
...
$$
\begin{aligned}
f_i&=\max_{j=i-k}^{L}\left\{f_j+\max_{l\in [j+1,i]}a_l\right\}\\\\
&=\max_{j=i-k}^{L}\left\{f_j+\max\left(\max_{l\in [j+1,L]}a_l,\max_{l\in[L+1,i]}a_l\right)\right\}\\\\
&=\max_{j=i-k}^{L}\left\{\max\left(f_j+\max_{l\in [j+1,L]}a_l,f_j+\max_{l\in[L+1,i]}a_l\right)\right\}\\\\
&=\max\left(\max_{j=i-k}^{L}\left\{f_j+\max_{l\in [j+1,L]}a_l\right\},\max_{j=i-k}^{L}\left\{f_j\right\}+\max_{l\in[L+1,i]}a_l\right)\\\\
\end{aligned}
$$
...
- 对于每一对 $(a,b)$，求有多少组可能的 $x,y$（只考虑 $x$ 的前 $a$ 位和 $y$ 的前 $b$ 位）满足 $\sum_{i=1}^a x_i=\sum_{j=1}^b y_j$。
...
- 给定以 $n$ 个字符串构成的字典 $D=\left\{s_1,s_2,\dots s_n\right\}$。
...
那么考虑如何用这个三元组统计答案。容易发现询问 $[l,r]$ 的答案即为 $\max_{l\le i<j\le r}\left\{\min(p,i-l+1)\right\}$。那么扫描线扫过去，把取 $p$ 的和取 $i-l+1$ 的分开维护即可。
