...
[传送门](https://www.luogu.com.cn/problem/AT_abc147_f)

> 题意

- 有一个长度为 $n$，首项为 $x$，公差为 $d$ 的等差数列 $a_i$。
- 设 $S\subseteq [1,n]$ 的权值为 $\sum_{i\in S}a_i-\sum_{i\notin S}a_i$。
- 求总共 $2^n$ 种 $S$ 有多少种不同的权值。
- $1\le n\le 2\times 10^5,-10^8\le x,d\le 10^8$

> 题解

很难不发现权值就是 $2\times \sum_{i\in S}a_i-\sum_{i=1}^na_i$，于是我们只需要考虑 $i\in S$ 的 $a_i$ 之和有多少种了。

容易想到枚举选了 $c$ 个，这样权值就大致是 $cx+td$ 的形式，不难发现 $t$ 的取值是一段连续的区间（并且最小最大值分别是序列前 $c$ 项和后 $c$ 项），那么容易想到按 $cx\bmod d$ 的值将 $i$ 分类，这样每一类就只需要关注 $d$ 的个数了（即 $\left\lfloor\frac{cx}{d}\right\rfloor+t$），于是随便维护一下区间即可，复杂度 $O(n\log n)$，瓶颈在各种排序离散化。
...
signed main(){
    ...
    ans=0;
    forup(i,0,n){
        i64 p=lower_bound(lsh.begin(),lsh.end(),(i*x)%d)-lsh.begin();
        i64 l=(i-1)*i/2,r=(n-i+n-1)*i/2;
        i64 t=(i*x)/d;
        vec[p].push_back(mkp(l+t,r+t));
    }
    forup(i,0,lsh.size()-1){
...
[传送门](https://www.luogu.com.cn/problem/CF436E)

> 题意

- 有 $n$ 个关卡，第 $i$ 个关卡可以花费 $a_i$ 时间获得一棵星星，或者花费 $b_i$ 时间获得两颗星星，或者不花费时间，但也不获得星星。
- 问获得 $w$ 颗星星所需最小时间。
- $1\le n\le 3\times 10^5,1\le w\le 2n,1\le a_i < b_i\le 10^9$

> 题解

第一眼感觉是反悔贪心，结果想了 1h 啥都不会。

容易发现对于 $b_i-a_i\ge a_i$ 是简单的，可以看作两个独立的物品，我们可以简单预处理**选出 $p$ 个的最小时间**。

考虑 $b_i-a_i < a_i$ 的情况，不难发现最多只会选一个单个的，假如我们选了两个单个的，那么将更大的去掉，另一个换成选两个必定不劣。

于是这个我们也能 $O(n)$ 维护，从小到大排序后预处理前缀和，如果选偶数个就直接是前缀和了，否则要么是前缀和加上后缀 $a_i$ 最小值，要么是前缀和减去前缀 $b_i-a_i$ 最大值。

然后我们枚举前者选了几个即可。复杂度 $O(n\log n)$，瓶颈在排序。
...
signed main(){
	...
	forup(i,1,n){
		...
		if(b[i]-a[i]>=a[i]){
			...
			vec.push_back(mkp(b[i]-a[i],i));
		}else{
			seq.push_back(mkp(b[i],i));
		}
	}
	...
	fordown(i,seq.size()-1,0){
		suf[i]=suf[i+1];
		if(!suf[i]||a[seq[i].se]<a[suf[i]]){
			suf[i]=seq[i].se;
		}
	}
	if(seq.size()){
	...
	forup(i,1,seq.size()-1){
		seq[i].fi+=seq[i-1].fi;
		pre[i]=pre[i-1];
		if(b[seq[i].se]-a[seq[i].se]>b[pre[i]]-a[pre[i]]){
			pre[i]=seq[i].se;
		}
	}
	ans=inf;
	int p=-1;
	forup(i,0,min(i64(2*seq.size()),m)){
		if(vec.size()<m-i) continue;
		i64 t=i/2,val;
		if(i&1){
			val=min(seq[t].fi-b[pre[t]]+a[pre[t]],(t==0?0:seq[t-1].fi)+a[suf[t]]);
		}else{
			val=(t==0?0:seq[t-1].fi);
		}
		val+=(i==m?0:vec[m-i-1].fi);
		if(val<ans){
			ans=val;p=i;
		}
	}
	...
	if(p&1){
		...
		if(seq[t].fi-b[pre[t]]+a[pre[t]]<(t==0?0:seq[t-1].fi)+a[suf[t]]){
			res[seq[t].se]=2;
			res[pre[t]]=1;
		}else{
			res[suf[t]]=1;
...
- 有一条长度为 $n$ 的链，链上每个点代表一家烤肉店。
- 有 $m$ 种烤肉，其中第 $i$ 家店的第 $j$ 种烤肉可以提供 $a_{i,j}$ 的幸福度。
- 每条边有一个 $d_i$ 表示经过这条边会减少多少幸福度。
- 求任选一个点作为起点，吃掉每一种烤肉各一份能获得的最大幸福度是多少。
- $1\le n\le 5000,1\le m\le 200,1\le d_i,a_{i,j}\le 10^9$

> 题解

容易发现其实就是找一个区间，用区间每种烤肉最大值之和减去区间长度。

容易发现后者直接前缀和搞定，前者相当于若干个矩形加（即找到每个 $a_{i,j}$ 成为最大值的区间左右端点区间），二维前缀和即可。
...
void work(int c){
	...
	a[0][c]=inf;
	forup(i,1,m){
		while(a[stk.top()][c]<a[i][c]) stk.pop();
		L[i]=stk.top();
		stk.push(i);
	}
	while(stk.size()) stk.pop();
	...
	forup(i,1,m){
		int xl=L[i]+1,xr=i,yl=i,yr=R[i]-1;
		sum[xl][yl]+=a[i][c];sum[xr+1][yl]-=a[i][c];
		sum[xl][yr+1]-=a[i][c];sum[xr+1][yr+1]+=a[i][c];
	}
}
signed main(){
	m=read();n=read();
...
- 有一张 $n$ 个点的有向图，初始为 $1\to 2\to 3\to \cdots \to n$ 的链。
- 你可以任意加 $m$ 条边。
- 求从点 $1$ 出发，每次从所有出边等概率随机走一条，走到 $n$ 的期望不是最大是多少。
- $1\le n\le 10^9,1\le m\le 10^{18}$，带多测，$1\le T\le 10^5$

> 题解

神秘题。

容易想到加的所有边都是从 $i\in [1,n-1]$ 连向 $1$ 的。

设点 $i$ 向 $1$ 连了 $a_i$ 条边，考虑从 $1$ 到 $i$ 的期望步数 $f_i$。

根据经典结论：发生概率为 $p$ 的事件发生一次所需尝试的期望次数为 $\frac{p}{1}$，容易发现：

- $f_1=0$
- $f_2=(f_1+1)(a_1+1)$
- $f_3=(f_2+1)(a_2+1)$

不难发现，$f_i=\sum_{j=1}^{i-1}\prod_{k=j}^{i-1}(a_k+1)$，将 $f$ 看作一个类似多项式的东西，考虑 $f_i$ 相当于对 $f_{i-1}$ 的所有项乘以 $a_{i-1}+1$ 再加上单独的一项 $a_{i-1}+1$。

容易想到 $a$ 应该大致是平均分的，并且显然是单调不减的，我们如果打表容易发现除去 $m < n-1$ 的情况（此时是平均分配），最优解必定是 $a_1$ 取某个值，然后连续一段 $a_i=a_1+1$，剩下的都是 $a_1+2$。但是咱不会证明。

于是等比数列求和搞一搞就做完了，复杂度 $O(T\log n)$。
...
i64 calc(i64 d,i64 q,i64 n){
    return d*(ksm(q,n)-1)%mod*ksm(q-1,mod-2)%mod;
}
int cnt=0;
void solve(){
    ...
    if(m<n-1){
        printf("%lld\n",((n-1-m)*ksm(2,m)%mod+calc(2,2,m))%mod);
        return;
    } 
    i64 a1=(m-(n-2))/(n-1),len=m-a1*(n-1)-(n-2);
    ++a1;
    a1=a1%mod;
    i64 res=(calc(a1+2,a1+2,len)+ksm(a1+2,len)*calc(a1+1,a1+1,n-2-len)%mod+a1*ksm(a1+1,n-2-len)%mod*ksm(a1+2,len)%mod)%mod;
    printf("%lld\n",res);
}
signed main(){
    i64 t=read();
...
## 模拟赛神秘题目 【1029 B组】D 传话游戏

> 题意

- 给定长度为 $m$ 的序列 $a$，现有一个由序列构成的长度为 $n$ 的序列 $S$，其中 $S_1=a$，并且对于所有 $i\ge 2$，满足 $S_i$ 是 $S_{i-1}$ 的子序列。
- 求有多少种本质不同的序列 $S$，我们认为 $S,S'$ 不同当且仅当存在 $i,j$，使得 $S_{i}$ 和 $S_{i}'$ 的第 $j$ 项不同。
- $1\le n\le 10^9,1\le a_i\le m\le 200$

> 题解

不是这谁他妈能想正解是 $O(m^4)$ 的区间 DP 啊。

首先容易想到求出每个 $a_i$ 的存活时间 $t_i$（即 $S_{t_i}$ 中有 $a_i$，但是 $S_{t_i+1}$ 中没有 $a_i$，此时我们姑且认为 $i\ne j,a_i=a_j$ 的 $a_i,a_j$ 是不同的）。

显然如果直接认为 $t_i$ 有 $n^m$ 种会算重，于是我们钦定连续的同一种颜色只保留前若干个，考虑哪些情况是合法的。

不难发现，对于 $a_i=a_j$，若 $\forall k\in [i+1,j-1]$ 均有 $a_k\ne a_i,t_k< \min(t_i,t_j)$ 并且 $t_i < t_j$，则不合法，因为会有一个时刻 $i,j$ 之间没有其它颜色，但是我们保留了 $j$。

于是我们反过来看，合法当且仅当对于所有 $i,j$ 满足 $a_i=a_j$ 且 $\forall k\in[i+1,j-1],a_k\ne a_i$，均有 $t_i\ge t_j$ 或者 $\exists k\in [i+1,j-1],t_k\ge \min(t_i,t_j)$。

接下来我们瞪大眼睛观察，容易发现这个条件等价于**对于所有 $i$，右侧第一个 $t_j > t_i$ 的 $j$ 颜色和 $i$ 不同**。

于是我们考虑构造 $t$ 的大根笛卡尔树，并且钦定每个子树的根是最左侧的区间最大值，那么容易发现这棵树合法当且仅当每个点 $i$ 左儿子的右链（一直走右儿子得到的链）颜色全都和 $i$ 不同。即笛卡尔树上**所有右端点为 $i-1$ 的子树的根**颜色和 $i$ 不同。

于是不难想到区间 DP，设 $f_{l,r,t}$ 表示考虑区间 $l,r$，区间最大值小于等于 $t$ 的方案数，转移就枚举树根即可，注意树根颜色不能和 $r+1$ 一样。

这玩意是 $O(nm^3)$ 的，但是不难发现 $f_{1,m,x}$ 应是一个关于 $x$ 的 $m$ 次多项式，证明可以考虑从 $l=r$ 的情况数学归纳法。于是只需要求出 $f_{1,m,0\sim m}$，然后插值即可，复杂度 $O(m^4)$。题解说:

![图示 2](../img/2024Oct_2.png)
...
void work(){
    forup(i,0,m){
        ...
        forup(j,0,m){
            if(j==i) continue;
            fordown(k,m,1){
                h[k]=(h[k-1]+mod-1ll*j*h[k]%mod)%mod;
            }
            h[0]=1ll*(mod-j)*h[0]%mod;
            pw=1ll*pw*ksm((mod+i-j)%mod,mod-2)%mod;
        }
        forup(i,0,m){
            (g[i]+=1ll*h[i]*pw%mod)%=mod;
        }
    }
}
signed main(){
    ...
    forup(len,1,m){
        forup(l,1,m-len+1){
            int r=l+len-1;
            forup(k,l,r){
                if(a[k]==a[r+1]) continue;
                forup(t,1,m){
                    (f[l][r][t]+=1ll*f[l][k-1][t-1]*f[k+1][r][t]%mod)%=mod;
                }
            }
            forup(t,1,m){
                (f[l][r][t]+=f[l][r][t-1])%=mod;
            }
        }
    }
    work();
...
[传送门](https://www.luogu.com.cn/problem/CF117E)

> 题意

- 有一张 $n$ 个点 $n$ 条边的无向图。初始所有边都是白色。
- 有 $m$ 个路径反转的操作，将 $u$ 到 $v$ 最短路上的边全部反转，若有多条最短路，**选择由路径上点编号构成的字符串字典序最小的**。
- 每次操作后，求由黑色边连接的连通块数。
- $1\le n,m\le 10^5$

> 题解

容易发现这张图是一棵树多一条边。那么我们先考虑树上怎么做。

容易发现答案是 $n$ 减去黑色边数，再考虑一下加上这条边会不会合并两连通块即可。

于是考虑怎么维护这条边会不会合并两连通块，显然不合并当且仅当两端点之间路径上所有边都是黑的，而修改是链反转，于是考虑树剖。

那么难点在比较两链的字典序，显然只需要比较两链的第一个分叉点即可，自己简单分讨即可，某条路径上某个点的“下一个点”可以树剖求。
...
vector<int> e[N];
int ff[N],dfn[N],Tm,hig[N],son[N],sz[N],dpt[N],mp[N];
void dfs1(int x,int fa){
	sz[x]=1;ff[x]=fa;
	dpt[x]=dpt[fa]+1;
	for(auto i:e[x]){
...
struct SegTree{
	...
	#define lson l,mid,id<<1
	#define rson mid+1,r,id<<1|1
	int cnt[N<<2],mark[N<<2];
	void PushUp(int id){
		cnt[id]=cnt[id<<1]+cnt[id<<1|1];
	}
	void modi(int l,int r,int id){
		cnt[id]=(r-l+1)-cnt[id];
		mark[id]^=1;
	}
...
int Query(int u,int v){
	...
	if(dpt[u]<dpt[v]) swap(u,v);
	if(u!=v) res+=mt.Query(dfn[v]+1,dfn[u]);
	return res;
}
void Update(int u,int v){
	while(hig[u]!=hig[v]){
		if(dpt[hig[u]]<dpt[hig[v]]) swap(u,v);
		mt.Update(dfn[hig[u]],dfn[u]);
		u=ff[hig[u]];
	}
	if(dpt[u]<dpt[v]) swap(u,v);
	if(u!=v) mt.Update(dfn[v]+1,dfn[u]);
}
int len,co;
int calc(){
	int res=n-mt.Query(1,Tm);
	if(co){
//		msg("[%d]\n",Query(ntu,ntv));
		if(Query(ntu,ntv)!=len){
			--res;
		}
	}
	return res;
}
int getpr(int u,int v){
	if(lca(u,v)==u){
		while(hig[v]!=hig[u]){
			v=hig[v];
			if(ff[v]==u) return v;
...
bool cmp(int x,int y,int tp){
	int u=ntu,v=ntv;
	if(tp) swap(u,v);
	if(dist(x,u)+dist(u,y)==dist(x,y)){
		return getpr(u,y)<v;
	}else{
		int xu=lca(x,u),xy=lca(x,y),uy=lca(u,v);
		int pp=xu;
		if(dpt[xy]>dpt[pp]) pp=xy;
		if(dpt[uy]>dpt[pp]) pp=uy;;
		return getpr(pp,y)<getpr(pp,u);
	}
}
signed main(){
	n=read();m=read();
	...
	forup(i,1,m){
		int x=read(),y=read();
		int d1=dist(x,y),d2=dist(x,ntu)+dist(y,ntv)+1,d3=dist(x,ntv)+dist(y,ntu)+1;
		msg("%d %d %d|\n",d1,d2,d3);
		if((d1<d2||(d1==d2&&cmp(x,y,0)))&&(d1<d3||(d1==d3&&cmp(x,y,1)))){
			msg("A");
			Update(x,y);
		}else{
			msg("B");
			if(d2<d3){
				Update(x,ntu);Update(y,ntv);
				co^=1;
			}else{
				Update(x,ntv);Update(y,ntu);
				co^=1;
			}
		}
		printf("%d\n",calc());
