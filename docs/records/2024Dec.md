...
[传送门](https://www.luogu.com.cn/problem/CF1930H)
...
[传送门](https://www.luogu.com.cn/problem/CF1585G)
...
[传送门](https://www.luogu.com.cn/problem/CF772E)
...
- 有一棵二叉树，上面有 $n$ 个叶子。
- 你可以进行至多 $10n$ 次以下询问：
    - 你给交互库输入 `a b c`，交互库返回 $X=\mathrm{lca}(a,b),Y=\mathrm{lca}(b,c),Z=\mathrm{lca}(c,a)$ 中最深的一个（即返回一个字符 `X` 或 `Y` 或 `Z`）。
- 输出这棵树，只要同构即可。
- $1\le n\le 1000$
